...
uint8_t __attribute__((aligned(4))) SDIO_DATA_BUFFER[512];
//#pragma pack(4)
...
//	SDIO_IRQn
	...
	NVIC->ISER[1] |= (SDIO_IRQn-32);

	NVIC->IP[SDIO_IRQn/4] |= (((0<<(4-2))|(0&(0x0f>>2)))<<((SDIO_IRQn%4)*8+4));
...
//		INTX_ENABLE();//开启总中断
	...
	{
  	    SD_DMA_Config((uint32_t*)buf,blksize,0);
		TransferError=SD_OK;
...
//nblks:要读取的块数
//返回值:错误状态
uint32_t __attribute__((aligned(4))) *tempbuff;
...
//		INTX_ENABLE();//开启总中断
	...
	{
		SD_DMA_Config((uint32_t*)buf,blksize,1);//SDIO DMA配置
   		TransferError=SD_OK;
		StopCondition=0;			//单块写,不需要发送停止传输指令
		TransferEnd=0;				//传输结束标置位，在中断服务置1
...
void SD_DMA_Config(uint32_t*mbuf,uint32_t bufsize,uint8_t dir)
{
	...
	DMA2_Channel4->CCR|=2<<12; 			//高优先级
 	...
 	DMA2_Channel4->CPAR=(uint32_t)&SDIO->FIFO;//DMA2 外设地址
	DMA2_Channel4->CMAR=(uint32_t)mbuf; 		//DMA2,存储器地址
