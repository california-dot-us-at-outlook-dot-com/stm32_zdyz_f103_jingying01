...
//内存池(32字节对齐)
uint8_t __attribute__((aligned(32))) mem1base[MEM1_MAX_SIZE];													//内部SRAM内存池
uint8_t __attribute__((aligned(32))) mem2base[MEM2_MAX_SIZE] __attribute__((section(".exram_start")));					//外部SRAM内存池
//内存管理表
uint16_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".exram_end")));	//外部SRAM内存池MAP
//内存管理参数
